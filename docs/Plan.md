...
| Release automation | DONE, `.goreleaser.yml` tag builds (linux + darwin) + SBOM + checksums, CI dry-run, `install.sh --from-release` sha256-verified |
| Self-update channels | DONE, `<HOME>/.tilde/install.json` records `kind` (`release`\|`source`); release installs (curl \| sh, --from-release) `tilde update` by downloading the newest tag, sha256-verifying it against `checksums.txt`, and swapping atomically (no downgrade); source installs pull+build; a missing record falls back to the release channel; `tilde doctor` reports the channel + install-dir writability |
| macOS sandbox | DONE, `internal/sandbox/seatbelt.go` runs shell calls under `sandbox-exec` (writes confined to the project root + temp, network denied unless opted in), fail-closed when sandbox-exec is missing; `Enforced`/`StatusLine` are OS-aware |
| Signed update verification | `tilde update` pulls and builds `origin/main` (not the tag's commit), so the release-tag signature check is advisory by default (warn and proceed); `TILDE_REQUIRE_SIGNED_TAGS=1` restores a hard refusal. The repo has no signing key, so the previous hard gate would have refused every future update. Historical unsigned tags never blocked refreshes |
...
| MCP user-authoritative merge | DONE, project config only adds servers or tightens per-tool approval; remote type + `headersFile`, SSRF-guarded URLs. Nested per-tool approval defaults to `prompt`, and the `mcp_call` gateway passes the outer ask-tier confirmation through as that approval (the user approves the exact `server.tool`), so MCP tools work by default; a raw `Manager.Call` without outer approval still refuses, and a project config cannot loosen approval toward auto |
...
| Ollama Cloud | DONE, optional-key backend; `tilde login ollama` / `/login ollama`, ladder store→env, `tilde doctor` readiness + remote-host-no-key warn, `/api/tags` validation |
...
| Edit discipline | DONE, warn-only; a wholesale rewrite of a substantial file or repeated edits to one path append a note to the tool result (`internal/tools`, `SeenMap.BumpEdits`) |
| Typed memory | DONE, `memory` save takes an optional kind (fact\|decision\|constraint\|env\|correction); `correct` supersedes matching lines and records a correction; recall states the authority rule; plain facts keep the untagged date-prefixed shape |

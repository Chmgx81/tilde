...
| Cloud onboarding (§2.24) | DONE (v1) — registry + ladder (flag>stored 0600>env), masked /login with validate-before-store, /logout, /model provider switching + catalog listing, budget auto-size unless explicit, command-shaped 401/404 hints; openrouter first-class with free-models shelf; gemini first-class (OpenAI-compatible endpoint, free tier); splash cloud list derives from registry; deferred: OAuth, keychain, native Gemini client, conditional reachability splash, closest-id 404 suggestion |

...
if [ -n "$FROM_RELEASE" ]; then
    ...
    if command -v curl >/dev/null 2>&1; then
        curl -fsSL --connect-timeout 10 --max-time 300 --retry 3 --retry-delay 2 -C - -o "$tmp/$base" "$rel/$base" \
        ...
        wget -q --timeout=20 --tries=3 -c -O "$tmp/$base" "$rel/$base" || die "download failed: $rel/$base"
